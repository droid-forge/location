...
/**
 * A {@link LocationProvider} that allows multiple location services to be used. <br>
 * <br>
...
public class MultiFallBackLocationProvider implements LocationProvider {
   ...
   * updates have already been started, this restarts location updates.<br>
   * <br>
   * If there are no location providers left, no action occurs.
   */
  ...
  /**
   * Builder class for the {@link MultiFallBackLocationProvider}.
   */
  public static class Builder {

    private MultiFallBackLocationProvider builtProvider;

    public Builder() {
      this.builtProvider = new MultiFallBackLocationProvider();
    }
    ...
    public Builder withGooglePlayServicesProvider() {
      return withServiceProvider(new GooglePlayServicesLocationProvider());
    }
    ...
    /**
     * Adds the given {@link ServiceLocationProvider} as a location provider. If the given location
     * provider detects that its underlying service is not available, the built <code>
     * MultiFallbackProvider</code> will fall back to the next location provider in the list.
     *
    ...
    public Builder withServiceProvider(ServiceLocationProvider provider) {
      FallbackListenerWrapper fallbackListener =
          new FallbackListenerWrapper(builtProvider, provider);
      provider.setServiceListener(fallbackListener);
      return withProvider(provider);
    }

    /**
     * Adds the given {@link LocationProvider} as a provider. Note that these providers <strong>DO
     * NOT</strong> support fallback behavior.
     *
    ...
    /**
     * Builds a {@link MultiFallBackLocationProvider} instance. If no providers were added to the builder,
     * the built-in Android Location Manager is used.
     */
    public MultiFallBackLocationProvider build() {
